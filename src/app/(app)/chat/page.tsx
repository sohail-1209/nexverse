...
interface Message {
  ...
  timestamp: Date;
  pdfFileName?: string;
  imageFileName?: string; // For images sent BY user
  imageUrl?: string; // For images sent/generated BY AI
}
...
const initialGreetingMessage: Message = { 
  id: 'ai-greeting', 
  text: "Hello! I'm NEXI ✨, your AI Assistant. How can I help you today? You can ask me questions, attach a PDF, send an image, or use voice input. I can also research online for you! To generate an image, type `/imagine <your prompt>`.", 
  sender: 'ai', 
  timestamp: new Date() 
};
...
export default function ChatPage() {
  ...
  const [capturedImage, setCapturedImage] = useState<string | null>(null); // Data URI for user captured image
  ...
  const handleSendMessage = async (e: FormEvent) => {
    ...
    try {
      ...
      if (imageCommandMatch) {
        ...
        if (!aiImageUrl && !aiText) { // If both are empty, provide a default error text
            aiText = "Sorry, I couldn't generate an image or provide a response for that prompt.";
        }
      } else if (capturedImage) { // If user sent an image
      ...
      } else { // Regular text query (possibly with PDF)
      ...
      const aiMessage: Message = {
        id: `ai-${Date.now()}`,
        text: aiText || "I'm not sure how to respond to that.", // Fallback text
        sender: 'ai',
        timestamp: new Date(),
        imageUrl: aiImageUrl,
      };
        ...
        {!isCapturing && ( 
            ...
            {(!showCameraView || (showCameraView && capturedImage && !isCapturing)) && ( 
                <form onSubmit={handleSendMessage} className="flex w-full items-center gap-3">
                    ...
                    <Input
                        type="text"
                        placeholder={isListening ? "Listening..." : (capturedImage ? "Add a query for the image..." : "Type /imagine <prompt> or message...")}
                        value={input}
